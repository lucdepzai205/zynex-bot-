...
# ================== THÔNG BÁO KEY TOÀN BỘ GROUP ==================
# /setnotifygroup do OWNER gửi riêng cho bot để bật thông báo cho TẤT CẢ
# group/supergroup mà bot đã nhìn thấy (đã nhận ít nhất một update).
NOTIFY_GROUPS_FILE = "notify_groups.json"
NOTIFY_SETTINGS_FILE = "notify_group_settings.json"

def _load_notify_groups():
    try:
        data = load_json(NOTIFY_GROUPS_FILE)
        if isinstance(data, list):
            return sorted({int(x) for x in data})
        if isinstance(data, dict):
            return sorted({int(x) for x in data.get("chat_ids", [])})
    except Exception as e:
        print(f"[NotifyGroup] Lỗi đọc danh sách group: {e}")
    return []

def _save_notify_groups(chat_ids):
    clean = sorted({int(x) for x in chat_ids})
    save_json(NOTIFY_GROUPS_FILE, clean)

def _notify_groups_enabled():
    try:
        data = load_json(NOTIFY_SETTINGS_FILE)
        return bool(data.get("enabled", False)) if isinstance(data, dict) else False
    except Exception:
        return False

def _set_notify_groups_enabled(enabled):
    save_json(NOTIFY_SETTINGS_FILE, {"enabled": bool(enabled)})

def _remember_group_chat(chat_id):
    try:
        chat_id = int(chat_id)
        if chat_id >= 0:
            return
        groups = _load_notify_groups()
        if chat_id not in groups:
            groups.append(chat_id)
            _save_notify_groups(groups)
            print(f"[NotifyGroup] Đã ghi nhận group: {chat_id}")
    except Exception as e:
        print(f"[NotifyGroup] Không lưu được group: {e}")

# Ghi nhận mọi group/supergroup mà bot nhận update. Không cần thêm handler
# riêng cho từng group. Nếu group chưa từng gửi update cho bot thì Telegram
# không cung cấp API để bot tự liệt kê toàn bộ lịch sử group đã tham gia.
_ORIG_PROCESS_NEW_MESSAGES = bot.process_new_messages
def _process_new_messages_track_groups(messages):
    try:
        for message in messages or []:
            chat = getattr(message, "chat", None)
            if chat and getattr(chat, "type", None) in ("group", "supergroup"):
                _remember_group_chat(chat.id)
    except Exception as e:
        print(f"[NotifyGroup] Lỗi theo dõi group: {e}")
    return _ORIG_PROCESS_NEW_MESSAGES(messages)

bot.process_new_messages = _process_new_messages_track_groups

@bot.message_handler(commands=['setnotifygroup'])
def handle_set_notify_group(msg):
    # Chỉ OWNER được bật/tắt cấu hình broadcast. Có thể gửi lệnh trong chat riêng.
    if msg.from_user.id != OWNER_ID:
        bot.reply_to(msg, "🔒 Chức năng này chỉ dành cho Admin.")
        return
    _set_notify_groups_enabled(True)
    groups = _load_notify_groups()
    bot.reply_to(
        msg,
        "✅ <b>ĐÃ BẬT THÔNG BÁO TOÀN BỘ GROUP</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        f"👥 Group đã ghi nhận: <b>{len(groups)}</b>\n"
        "📢 Từ giờ thông báo sẽ gửi tới tất cả group bot đã ghi nhận.\n"
        "➕ Bot vào group mới và nhận được tin nhắn thì group đó cũng tự động được thêm.",
        parse_mode="HTML"
    )
...
def handle_notify_group(msg):
    if msg.from_user.id != OWNER_ID:
        ...
        return
    groups = _load_notify_groups()
    status = "🟢 ĐANG BẬT" if _notify_groups_enabled() else "🔴 ĐANG TẮT"
    if not groups:
        detail = "Chưa ghi nhận group nào."
    else:
        detail = "\n".join(f"• <code>{gid}</code>" for gid in groups)
    bot.reply_to(
        msg,
        f"📢 <b>THÔNG BÁO GROUP</b>\n{status}\n👥 Số group: <b>{len(groups)}</b>\n\n{detail}",
        parse_mode="HTML"
    )
...
def handle_unset_notify_group(msg):
    if msg.from_user.id != OWNER_ID:
        bot.reply_to(msg, "🔒 Chức năng này chỉ dành cho Admin.")
        return
    _set_notify_groups_enabled(False)
    bot.reply_to(msg, "🔴 <b>ĐÃ TẮT THÔNG BÁO TOÀN BỘ GROUP.</b>", parse_mode="HTML")

def _send_key_notify_group(text):
    """Broadcast thông báo Key tới tất cả group đã ghi nhận."""
    if not _notify_groups_enabled():
        return 0, 0
    groups = _load_notify_groups()
    sent = 0
    failed = 0
    ...
    for chat_id in groups:
        try:
            ...
            failed += 1
            print(f"[NotifyGroup] Không gửi được group {chat_id}: {e}")
            # Giữ group trong danh sách để không tự mất cấu hình khi Telegram tạm lỗi.
    return sent, failed
...
def change_balance(user_id, amount):
    ...
    uid = str(user_id)
    new_value = int(balances.get(uid, 0)) + int(amount)
    if new_value < 0:
        return False, int(balances.get(uid, 0))
    balances[uid] = new_value
...
def handle_muakey(msg):
    ...
    text = (
        ...
        "Nhap lenh:\n"
        "/buy + thoi gian\n"
        "Vi du: /buy1day /buy3day /buy1week /buy1month /buyvip"
    )
...
def handle_done(msg):
    ...
    # Đơn NẠP TIỀN: /done sẽ cộng tiền vào ví, KHÔNG tạo key.
    if order_info.get("type") == "deposit" or order_info.get("key_type") == "deposit":
        user_id = int(order_info.get("user_id"))
        amount = int(order_info.get("amount", 0))
        if amount <= 0:
            bot.reply_to(msg, f"❌ Đơn {order_code} có số tiền không hợp lệ.")
            return
        ok, new_balance = change_balance(user_id, amount)
        if not ok:
            bot.reply_to(msg, "❌ Không thể cập nhật số dư.")
            return
        now = datetime.now()
        order_info["status"] = "done"
        order_info["done_at"] = now.isoformat()
        order_info["approved_by"] = msg.from_user.id
        order_info["balance_after"] = new_balance
        ...
        bot.send_message(
            user_id,
            "✅ <b>DUYỆT NẠP TIỀN THÀNH CÔNG</b>\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"🧾 <b>Đơn:</b> <code>{order_code}</code>\n"
            f"👤 <b>Tên:</b> {display_name}\n"
            f"💰 <b>Số tiền:</b> {format_money(amount)}\n"
            f"💵 <b>Số dư mới:</b> {format_money(new_balance)}\n"
            "━━━━━━━━━━━━━━━━━━\n💜 <b>ZYNEX AI</b>",
            parse_mode="HTML"
        )
        bot.reply_to(
...
BETVIP_API_URL = "https://betvip2x.hacksieucap.pro/huddd"
BETVIP_RUNNING = {}
BETVIP_LAST_SESSION = {}
BETVIP_STATS_FILE = "betvip_stats.json"
BETVIP_STATS = {}


def _load_betvip_stats():
    global BETVIP_STATS
    try:
        with open(BETVIP_STATS_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            BETVIP_STATS = data if isinstance(data, dict) else {}
    except Exception:
        BETVIP_STATS = {}


def _save_betvip_stats():
    try:
        with open(BETVIP_STATS_FILE, "w", encoding="utf-8") as f:
            json.dump(BETVIP_STATS, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"[BETVIP STATS] Lỗi lưu thống kê: {e}")


def _betvip_stats(chat_id):
    key = str(chat_id)
    if key not in BETVIP_STATS:
        BETVIP_STATS[key] = {
            "correct": 0,
            "wrong": 0,
            "pending": None,
    ...
    return BETVIP_STATS[key]


def _betvip_result(data):
    value = str(data.get("Ket_qua", data.get("ket_qua", ""))).strip().lower()
    if value in {"t", "tai", "tài"}:
        return "Tài"
    if value in {"x", "xiu", "xỉu"}:
        return "Xỉu"
    try:
        total = int(data.get("Tong", data.get("tong")))
        if total >= 11:
            return "Tài"
        if total >= 3:
            return "Xỉu"
...
def _betvip_prediction(data):
    value = str(data.get("du_doan", data.get("prediction", ""))).strip().lower()
    if value in {"t", "tai", "tài"}:
        return "Tài"
    if value in {"x", "xiu", "xỉu"}:
        return "Xỉu"
...
def _betvip_grade(chat_id, data):
    st = _betvip_stats(chat_id)
    pending = st.get("pending")
    if not pending:
        return

    current_session = str(data.get("Phien", data.get("phien", "?")))
    if current_session == str(pending.get("session")):
        return

    prediction = pending.get("prediction")
    actual = _betvip_result(data)
    if prediction not in {"Tài", "Xỉu"} or actual not in {"Tài", "Xỉu"}:
        st["pending"] = None
        _save_betvip_stats()
        return
...
def _betvip_stats_text(chat_id):
    st = _betvip_stats(chat_id)
    correct = int(st.get("correct", 0))
    wrong = int(st.get("wrong", 0))
    total = correct + wrong
    rate = correct * 100 / total if total else 0
    ...
    if history:
        lines += ["━━━━━━━━━━━━━━━━━━", "📜 <b>30 phiên gần nhất:</b>"]
        for item in history[:30]:
            icon = "✅" if item.get("status") == "ĐÚNG" else "❌"
            lines.append(
                f"{icon} #{item.get('session', '?')} | "
                f"Dự: {item.get('prediction', '?')} | "
                f"KQ: {item.get('actual', '?')}"
            )
    return "\n".join(lines)
...
def betvip_keyboard():
    kb = tg_types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    kb.row(
        tg_types.KeyboardButton("🟢 BẬT BETVIP"),
        tg_types.KeyboardButton("🔴 TẮT BETVIP"),
    )
    kb.row(tg_types.KeyboardButton("🔄 BETVIP CẬP NHẬT"))
    kb.row(tg_types.KeyboardButton("📊 THỐNG KÊ BETVIP"))
    kb.row(tg_types.KeyboardButton("↩️ Quay lại Menu Game"))
    return kb


def _betvip_format(data, chat_id=None):
    if not isinstance(data, dict):
        return "❌ BETVIP trả về dữ liệu không hợp lệ."

    phien = data.get("Phien", data.get("phien", "?"))
    tong = data.get("Tong", data.get("tong", "?"))
    d1 = data.get("Xuc_xac_1", data.get("xuc_xac_1", "?"))
    d2 = data.get("Xuc_xac_2", data.get("xuc_xac_2", "?"))
    d3 = data.get("Xuc_xac_3", data.get("xuc_xac_3", "?"))
    ketqua = _betvip_result(data)
    du_doan = _betvip_prediction(data)
    ty_le = data.get("ty_le_dd", "?")
    chien_thuat = data.get("chien_thuat", "?")
    dang_cau = data.get("dang_cau", "?")
    phien_du_doan = data.get("phiendudoan", "?")
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        f"🎯 Phiên: <b>{phien}</b>\n"
        f"🎲 Xúc xắc: <b>{d1}-{d2}-{d3}</b> | Tổng: <b>{tong}</b>\n"
        f"🏆 Kết quả: <b>{ketqua}</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        f"🔮 Phiên dự đoán: <b>{phien_du_doan}</b>\n"
        f"🤖 Dự đoán: <b>{du_doan}</b>\n"
        f"🎯 Tỷ lệ tham khảo: <b>{ty_le}</b>\n"
        f"🧠 Chiến thuật: <b>{chien_thuat}</b>\n"
        f"📊 Dạng cầu: <b>{dang_cau}</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        f"🕒 Cập nhật: <code>{timestamp}</code>\n"
    )
    if chat_id is not None:
        text += "━━━━━━━━━━━━━━━━━━\n" + _betvip_stats_text(chat_id)
    return text
...
def _betvip_loop(chat_id):
    while BETVIP_RUNNING.get(chat_id):
        try:
            data = fetch_betvip()
            phien = data.get("Phien", data.get("phien")) if isinstance(data, dict) else None
            if phien is not None and BETVIP_LAST_SESSION.get(chat_id) != str(phien):
                _betvip_grade(chat_id, data)

                prediction = _betvip_prediction(data)
                if prediction in {"Tài", "Xỉu"}:
                    _betvip_stats(chat_id)["pending"] = {
                        "session": str(phien),
                        "prediction": prediction
                    }
                    _save_betvip_stats()
                ...
                bot.send_message(
                    chat_id,
                    _betvip_format(data, chat_id),
                    parse_mode="HTML"
                )
                BETVIP_LAST_SESSION[chat_id] = str(phien)
        except Exception as e:
            print(f"[BETVIP API] Lỗi lấy dữ liệu: {e}")
        time.sleep(3)


@bot.message_handler(func=lambda m: m.text == "🎯 GAME BETVIP")
def game_betvip_button(msg):
    if not _group_admin_only(msg):
        return
    if msg.from_user.id in kicked_users:
        bot.reply_to(msg, "🚫 Bạn đã bị chặn!")
        return
    show_keyboard(msg.chat.id, betvip_keyboard())


@bot.message_handler(func=lambda m: m.text == "🟢 BẬT BETVIP")
def betvip_start_button(msg):
    if not _group_admin_only(msg):
        return
    chat_id = msg.chat.id
    if BETVIP_RUNNING.get(chat_id):
        bot.send_message(chat_id, "🟢 BETVIP đang chạy.")
        return
    BETVIP_RUNNING[chat_id] = True
    BETVIP_LAST_SESSION.pop(chat_id, None)
    bot.send_message(
        chat_id,
        "🟢 <b>BETVIP: ĐÃ BẬT.</b>\n📡 Trạng thái: <b>ĐÃ KẾT NỐI</b>.",
        parse_mode="HTML"
    )
    threading.Thread(target=_betvip_loop, args=(chat_id,), daemon=True).start()


@bot.message_handler(func=lambda m: m.text == "🔴 TẮT BETVIP")
def betvip_stop_button(msg):
    if not _group_admin_only(msg):
        return
    BETVIP_RUNNING[msg.chat.id] = False
    bot.send_message(msg.chat.id, "🔴 <b>BETVIP: ĐÃ TẮT.</b>", parse_mode="HTML")


@bot.message_handler(func=lambda m: m.text == "📊 THỐNG KÊ BETVIP")
def betvip_stats_button(msg):
    if not _group_admin_only(msg):
        return
    bot.send_message(msg.chat.id, _betvip_stats_text(msg.chat.id), parse_mode="HTML")


@bot.message_handler(func=lambda m: m.text == "🔄 BETVIP CẬP NHẬT")
def betvip_update_button(msg):
    if not _group_admin_only(msg):
        return
    try:
        data = fetch_betvip()
        _betvip_grade(msg.chat.id, data)

        prediction = _betvip_prediction(data)
        if prediction in {"Tài", "Xỉu"}:
            _betvip_stats(msg.chat.id)["pending"] = {
                "session": str(data.get("Phien", data.get("phien", "?"))),
                "prediction": prediction
            }
            _save_betvip_stats()
        ...
        bot.send_message(
            msg.chat.id,
            _betvip_format(data, msg.chat.id),
            parse_mode="HTML"
        )
    except Exception as e:
        # Không hiển thị URL API cho người dùng.
        bot.send_message(
            msg.chat.id,
            f"❌ Không lấy được dữ liệu BETVIP: <code>{e}</code>",
            parse_mode="HTML"
        )
...
def key_main_keyboard():
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("💳 Mua Key bằng Số Dư", callback_data="key_buy_balance"))
    kb.add(types.InlineKeyboardButton("💰 Bảng Giá Key", callback_data="key_prices"))
    kb.add(types.InlineKeyboardButton("💵 Số Dư", callback_data="key_balance"))
    kb.add(types.InlineKeyboardButton("↩️ Quay lại Menu chính", callback_data="key_back_main"))
    return kb

def key_prices_keyboard():
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("⭐ 1 Ngày — 35.000đ", callback_data="buy_1ngay"))
    kb.add(types.InlineKeyboardButton("⭐ 3 Ngày — 55.000đ", callback_data="buy_3ngay"))
    kb.add(types.InlineKeyboardButton("⭐ 1 Tuần — 85.000đ", callback_data="buy_1tuan"))
    kb.add(types.InlineKeyboardButton("⭐ 1 Tháng — 125.000đ", callback_data="buy_1thang"))
    kb.add(types.InlineKeyboardButton("👑 Vĩnh Viễn — 300.000đ", callback_data="buy_vip"))
    kb.add(types.InlineKeyboardButton("↩️ Quay lại Menu Key", callback_data="key_menu"))
    return kb


def payment_keyboard(order_code):
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("🔄 Tôi đã chuyển khoản", callback_data=f"paid_{order_code}"))
    kb.add(types.InlineKeyboardButton("↩️ Quay lại chọn loại Key", callback_data="key_prices"))
    return kb
...
@bot.callback_query_handler(func=lambda call: call.data == "key_balance")
def key_balance_callback(call):
    if not _group_admin_only(call.message):
        bot.answer_callback_query(call.id)
        return
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id, f"💵 <b>SỐ DƯ TÀI KHOẢN</b>\n━━━━━━━━━━━━━━━━━━\n💰 Số dư hiện tại: <b>{format_money(get_balance(call.from_user.id))}</b>", parse_mode="HTML")


@bot.callback_query_handler(func=lambda call: call.data == "key_buy_balance")
def key_buy_balance_callback(call):
    if not _group_admin_only(call.message):
        bot.answer_callback_query(call.id)
        return
    bot.answer_callback_query(call.id)
    bot.send_message(
        call.message.chat.id,
        "💳 <b>MUA KEY BẰNG SỐ DƯ</b>\n━━━━━━━━━━━━━━━━━━\n💵 Số dư: <b>" + format_money(get_balance(call.from_user.id)) + "</b>\n\n👇 Chọn gói Key:",
        parse_mode="HTML", reply_markup=balance_buy_keyboard()
    )


def balance_buy_keyboard():
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("⭐ 1 Ngày — 35.000đ", callback_data="wallet_buy_1ngay"))
    kb.add(types.InlineKeyboardButton("⭐ 3 Ngày — 55.000đ", callback_data="wallet_buy_3ngay"))
    kb.add(types.InlineKeyboardButton("⭐ 1 Tuần — 85.000đ", callback_data="wallet_buy_1tuan"))
    kb.add(types.InlineKeyboardButton("⭐ 1 Tháng — 125.000đ", callback_data="wallet_buy_1thang"))
    kb.add(types.InlineKeyboardButton("👑 Vĩnh Viễn — 300.000đ", callback_data="wallet_buy_vip"))
    kb.add(types.InlineKeyboardButton("↩️ Quay lại Menu Key", callback_data="key_menu"))
    return kb


@bot.callback_query_handler(func=lambda call: call.data.startswith("wallet_buy_"))
def wallet_buy_callback(call):
    if not _group_admin_only(call.message):
        bot.answer_callback_query(call.id)
        return
    packages = {
        "wallet_buy_1ngay": ("1ngay", "KEY 1 NGÀY", 35000, 1),
        "wallet_buy_3ngay": ("3ngay", "KEY 3 NGÀY", 55000, 3),
        "wallet_buy_1tuan": ("1tuan", "KEY 1 TUẦN", 85000, 7),
        "wallet_buy_1thang": ("1thang", "KEY 1 THÁNG", 125000, 30),
        "wallet_buy_vip": ("vinhvien", "KEY VĨNH VIỄN", 300000, 100000),
    }
    key_type, display_name, price, days = packages[call.data]
    uid = call.from_user.id
    balance = get_balance(uid)
    if balance < price:
        bot.answer_callback_query(call.id, "Số dư không đủ", show_alert=True)
        bot.send_message(call.message.chat.id, f"❌ <b>SỐ DƯ KHÔNG ĐỦ</b>\n\n💵 Hiện có: <b>{format_money(balance)}</b>\n💰 Cần: <b>{format_money(price)}</b>\n📌 Thiếu: <b>{format_money(price-balance)}</b>", parse_mode="HTML")
        return
    ok, new_balance = change_balance(uid, -price)
    if not ok:
        bot.answer_callback_query(call.id, "Không thể trừ số dư", show_alert=True)
        return
    now = datetime.now()
    ...
    keys[key] = expire_time.isoformat()
    save_keys(keys)
    authenticated_users[uid] = expire_time
    save_auth_users_file()
    bot.answer_callback_query(call.id, "Mua Key thành công")
    bot.send_message(call.message.chat.id,
        "🎉 <b>MUA KEY THÀNH CÔNG</b>\n━━━━━━━━━━━━━━━━━━\n"
        f"🎁 Gói: <b>{display_name}</b>\n"
        f"💰 Giá: <b>{format_money(price)}</b>\n"
        f"💵 Số dư còn lại: <b>{format_money(new_balance)}</b>\n"
        f"🔑 Key: <code>{key}</code>\n"
        f"🕐 Kích hoạt: {now.strftime('%d/%m/%Y %H:%M')}\n"
        f"⏳ Hết hạn: {expire_time.strftime('%d/%m/%Y %H:%M')}", parse_mode="HTML")
...
def key_prices_callback(call):
    ...
    try:
        bot.edit_message_text(
            ...
            "⭐ 3 NGÀY — <b>55.000đ</b>\n"
            "⭐ 1 TUẦN — <b>85.000đ</b>\n"
            "⭐ 1 THÁNG — <b>125.000đ</b>\n"
            "👑 VĨNH VIỄN — <b>300.000đ</b>\n\n"
...
@bot.callback_query_handler(func=lambda call: call.data in {
    "buy_1ngay", "buy_3ngay", "buy_1tuan", "buy_1thang", "buy_vip"
})
def buy_package_callback(call):
    ...
    packages = {
        "buy_1ngay": ("1ngay", "KEY 1 NGÀY", "35.000đ"),
        "buy_3ngay": ("3ngay", "KEY 3 NGÀY", "55.000đ"),
        "buy_1tuan": ("1tuan", "KEY 1 TUẦN", "85.000đ"),
        "buy_1thang": ("1thang", "KEY 1 THÁNG", "125.000đ"),
        "buy_vip": ("vinhvien", "KEY VĨNH VIỄN", "300.000đ"),
    }
...
def deposit_menu_keyboard():
    ...
    kb.add(
        types.InlineKeyboardButton("🏦 Chuyển Khoản", callback_data="deposit_bank"),
        types.InlineKeyboardButton("🎫 Thẻ Cào", callback_data="deposit_card")
    )
    kb.add(types.InlineKeyboardButton("↩️ Quay Lại", callback_data="deposit_back"))
    return kb


def deposit_menu_text():
    return (
        "💳 <b>NẠP TIỀN</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "💰 Chọn phương thức nạp tiền bên dưới:\n\n"
        "🏦 <b>Chuyển Khoản</b> — nhập số tiền, bot tạo mã QR và mã nội dung riêng.\n"
        "🎫 <b>Thẻ Cào</b> — gửi thông tin thẻ cho Admin kiểm tra.\n\n"
        "👇 Chọn phương thức:"
    )


def _deposit_payment_keyboard(order_code):
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("🔄 Tôi đã chuyển khoản", callback_data=f"paid_deposit_{order_code}"))
    kb.add(types.InlineKeyboardButton("↩️ Nạp Tiền", callback_data="deposit_back"))
    return kb


def _save_deposit_order(user_id, amount, order_code):
    orders = load_orders()
    orders[order_code] = {
        "user_id": int(user_id),
        "type": "deposit",
...
@bot.callback_query_handler(func=lambda call: call.data == "deposit_bank")
def deposit_bank_callback(call):
    if not _group_admin_only(call.message):
        bot.answer_callback_query(call.id)
        return
    bot.answer_callback_query(call.id)
    uid = call.from_user.id
    PENDING_DEPOSIT_USERS.add(uid)
    bot.send_message(
        call.message.chat.id,
        "🏦 <b>NẠP TIỀN BẰNG CHUYỂN KHOẢN</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "💰 Nhập <b>số tiền muốn nạp</b> bằng số nguyên.\n\n"
        "Ví dụ: <code>100000</code> hoặc <code>500000</code>\n\n"
        "📌 Sau khi nhập, bot sẽ tạo <b>mã QR + nội dung chuyển khoản riêng</b> cho bạn.",
        parse_mode="HTML"
    )


@bot.callback_query_handler(func=lambda call: call.data == "deposit_card")
def deposit_card_callback(call):
    if not _group_admin_only(call.message):
        bot.answer_callback_query(call.id)
        return
    bot.answer_callback_query(call.id)
    bot.send_message(
        call.message.chat.id,
        "🎫 <b>NẠP TIỀN BẰNG THẺ CÀO</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "📌 Gửi cho Admin:\n"
        "• Nhà mạng\n• Mệnh giá\n• Mã thẻ\n• Serial\n\n"
        "📸 Sau khi gửi, chờ Admin kiểm tra và cộng tiền.\n"
        "👤 <b>Admin:</b> @luc_z2005",
        parse_mode="HTML",
...
@bot.callback_query_handler(func=lambda call: call.data == "deposit_back")
def deposit_back_callback(call):
    if not _group_admin_only(call.message):
        bot.answer_callback_query(call.id)
        return
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text(
            deposit_menu_text(), call.message.chat.id, call.message.message_id,
            parse_mode="HTML", reply_markup=deposit_menu_keyboard()
        )
    except Exception:
        bot.send_message(call.message.chat.id, deposit_menu_text(), parse_mode="HTML", reply_markup=deposit_menu_keyboard())


@bot.message_handler(func=lambda m: m.text and m.from_user.id in PENDING_DEPOSIT_USERS and not m.text.startswith('/'))
def process_deposit_amount(msg):
    if not _group_admin_only(msg):
        return
    uid = msg.from_user.id
    raw = msg.text.strip().replace('.', '').replace(',', '').replace(' ', '')
    try:
        amount = int(raw)
    except ValueError:
        bot.send_message(msg.chat.id, "❌ Số tiền không hợp lệ. Ví dụ: <code>100000</code>", parse_mode="HTML")
        return
    if amount < 1000:
        bot.send_message(msg.chat.id, "❌ Số tiền nạp tối thiểu là <b>1.000đ</b>.", parse_mode="HTML")
        return
    if amount > 100000000:
        bot.send_message(msg.chat.id, "❌ Số tiền nạp tối đa là <b>100.000.000đ</b>.", parse_mode="HTML")
        return

    PENDING_DEPOSIT_USERS.discard(uid)
    order_code = make_order_code()
    _save_deposit_order(uid, amount, order_code)
    ...
    qr_url = (
        "https://img.vietqr.io/image/"
        f"VCB-{stk}-compact2.png?amount={amount}"
        f"&addInfo={order_code}&accountName={receiver.replace(' ', '%20')}"
    )
    caption = (
        "🏦 <b>NẠP TIỀN THÀNH CÔNG - TẠO ĐƠN</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        f"💰 <b>Số tiền nạp:</b> {format_money(amount)}\n"
        f"🧾 <b>Mã đơn:</b> <code>{order_code}</code>\n\n"
        "📌 Quét QR bên trên hoặc chuyển khoản theo thông tin:\n\n"
        f"🏦 <b>Ngân hàng:</b> {bank}\n"
        f"👤 <b>Chủ TK:</b> {receiver}\n"
        f"💳 <b>STK:</b> <code>{stk}</code>\n"
        f"📝 <b>Nội dung CK:</b> <code>{order_code}</code>\n\n"
        "📸 Chuyển khoản xong gửi bill cho Admin để kiểm tra và cộng số dư.\n"
        "👤 <b>Admin:</b> @luc_z2005"
    )
    try:
        bot.send_photo(msg.chat.id, qr_url, caption=caption, parse_mode="HTML", reply_markup=_deposit_payment_keyboard(order_code))
    except Exception:
        bot.send_message(msg.chat.id, caption, parse_mode="HTML", reply_markup=_deposit_payment_keyboard(order_code))


@bot.callback_query_handler(func=lambda call: call.data.startswith("paid_deposit_"))
def paid_deposit_callback(call):
    if not _group_admin_only(call.message):
        bot.answer_callback_query(call.id)
        return
    order_code = call.data[len("paid_deposit_"):]
    bot.answer_callback_query(call.id, "Đã ghi nhận. Admin sẽ kiểm tra.")
    bot.send_message(
        call.message.chat.id,
        f"📨 <b>ĐÃ BÁO CHUYỂN KHOẢN</b>\nMã đơn: <code>{order_code}</code>\n\n"
        "Vui lòng gửi bill cho Admin @luc_z2005. Sau khi kiểm tra, Admin dùng <code>/done " + order_code + "</code> để cộng số dư.",
        parse_mode="HTML"
    )
...
def deposit_button(msg):
    if not _group_admin_only(msg):
        return
    PENDING_DEPOSIT_USERS.discard(msg.from_user.id)
    bot.send_message(msg.chat.id, deposit_menu_text(), parse_mode="HTML", reply_markup=deposit_menu_keyboard())
...
def balance_button(msg):
    ...
    bot.send_message(
        msg.chat.id,
        f"💵 <b>SỐ DƯ TÀI KHOẢN</b>\n━━━━━━━━━━━━━━━━━━\n💰 Số dư hiện tại: <b>{format_money(get_balance(msg.from_user.id))}</b>",
        parse_mode="HTML"
    )
...
def handle_thongbao(msg):
    ...
    # Broadcast cùng nội dung vào toàn bộ group đã ghi nhận. Không để lộ dữ liệu/key của khách.
    group_sent, group_failed = _send_key_notify_group(notice)
    failed += group_failed

    bot.reply_to(
        msg,
        f"✅ <b>Đã gửi thông báo.</b>\n👥 Khách nhận: <b>{sent}</b>\n⚠️ Lỗi: <b>{failed}</b>\n📢 Group nhận: <b>{group_sent}</b>",
        parse_mode="HTML"
    )
